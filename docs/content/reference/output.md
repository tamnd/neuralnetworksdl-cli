...
describes what users get. Pick a format with `-o`, or let neuralnetworksdl choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
neuralnetworksdl <command> -o table   # aligned columns for reading
neuralnetworksdl <command> -o jsonl   # one JSON object per line, for piping
neuralnetworksdl <command> -o json    # a single JSON array
neuralnetworksdl <command> -o csv     # spreadsheet friendly
neuralnetworksdl <command> -o tsv     # tab-separated
neuralnetworksdl <command> -o url     # just the URL column
neuralnetworksdl <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
neuralnetworksdl <command> --fields id,title,url
```
...
```bash
neuralnetworksdl <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
neuralnetworksdl <command>            # a table, because this is a terminal
neuralnetworksdl <command> | wc -l    # JSONL, because this is a pipe
```
