...
RUN apk add --no-cache ca-certificates tzdata \
 && adduser -D -H -u 10001 neuralnetworksdl

COPY $TARGETPLATFORM/neuralnetworksdl /usr/bin/neuralnetworksdl

USER neuralnetworksdl

ENTRYPOINT ["/usr/bin/neuralnetworksdl"]
