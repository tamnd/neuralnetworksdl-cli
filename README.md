...
`neuralnetworksdl` is a single pure-Go binary. It reads public neuralnetworksdl data
...
neuralnetworksdl page <path> --template '{{.Body}}'  # just the readable body text
neuralnetworksdl links <path>                     # the pages it links to, one per line
...
to end. Model the real neuralnetworksdl records in `neuralnetworksdl/` and declare their
operations in `neuralnetworksdl/domain.go`; each one becomes a command, an HTTP
...
```bash
neuralnetworksdl serve --addr :7777    # GET /v1/page/<path>  returns NDJSON
...
dereferences `neuralnetworksdl://` URIs without knowing anything about neuralnetworksdl:
